...
std::string ToString(Antomic::stmt_t node)
{
	switch(node->stmt_kind)
	{
		case Antomic::stmt_::StmtKind::kAssert: {
			...
			return IDENTATION() + "Assert(" + ToString(stmt->test) +
				   (stmt->msg ? "," + ToString(stmt->msg) : "") + ")" + NEWLINE();
		}
		case Antomic::stmt_::StmtKind::kAssign: {
			auto stmt = dynamic_cast<Antomic::Assign_t>(node);
			return IDENTATION() + "Assign(" + ToString(stmt->target) + "," + ToString(stmt->value) +
				   ")" + NEWLINE();
		}
		case Antomic::stmt_::StmtKind::kAugAssign: {
			auto stmt = dynamic_cast<Antomic::AugAssign_t>(node);
			return IDENTATION() + "AssignAug(" + ToString(stmt->target) + "," + ToString(stmt->op) +
				   "," + ToString(stmt->value) + ")" + NEWLINE();
		}
		...
		case Antomic::stmt_::StmtKind::kClassDef: {
			...
			auto result = IDENTATION() + "Class(" + stmt->name + "," + ToString(stmt->bases) +
						  "):" + NEWLINE();
			INDENT();
		...
		case Antomic::stmt_::StmtKind::kFor: {
			...
			auto result = IDENTATION() + "For(" + ToString(stmt->target) + "," +
						  ToString(stmt->iter) + "):" + NEWLINE();
			INDENT();
			List(stmt->body, result);
			DEDENT();
			return result;
		...
		case Antomic::stmt_::StmtKind::kFunctionDef: {
			...
			auto result = IDENTATION() + "FunctionDef(" + stmt->name + "," + ToString(stmt->args) +
						  "):" + NEWLINE();
			INDENT();
			List(stmt->body, result);
			DEDENT();
		...
		case Antomic::stmt_::StmtKind::kImportFrom: {
			...
			return IDENTATION() + "ImportFrom(" + stmt->module + "," + ToString(stmt->names) + ")" +
				   NEWLINE();
		}
		...
		case Antomic::stmt_::StmtKind::kReturn: {
			...
			return IDENTATION() + "Return(" + (stmt->value ? ToString(stmt->value) : "") + ")" +
				   NEWLINE();
		}
...
std::string ToString(Antomic::expr_t node)
{
	...
	switch(node->expr_kind)
	{
		...
		case Antomic::expr_::ExprKind::kBinOp: {
			...
			return "BinOp(" + ToString(expr->left) + "," + ToString(expr->op) + "," +
				   ToString(expr->right) + ")";
		}
		case Antomic::expr_::ExprKind::kBoolOp: {
			auto expr = dynamic_cast<Antomic::BoolOp_t>(node);
			return "BoolOp(" + ToString(expr->left) + "," + ToString(expr->op) + "," +
				   ToString(expr->right) + ")";
		}
		...
		case Antomic::expr_::ExprKind::kCompare: {
			...
			return "Compare(" + ToString(expr->left) + "," + ToString(expr->op) + "," +
				   ToString(expr->right) + ")";
		}
		...
		case Antomic::expr_::ExprKind::kDict: {
			...
			for(size_t i = 0; i < expr->keys.size(); i++)
			{
				...
				result +=
					"KeyValue(" + ToString(expr->keys[i]) + "," + ToString(expr->values[i]) + ")";
			}
			result += ")";
			return result;
		}
		...
		case Antomic::expr_::ExprKind::kSlice: {
			...
			return "Slice(Lower(" + (expr->lower ? ToString(expr->lower) : "") + "),Upper(" +
				   (expr->upper ? ToString(expr->upper) : "") + "),Step(" +
				   (expr->step ? ToString(expr->step) : "") + "))";
		}
...
std::string ToString(Antomic::excepthandler_t node)
{
	...
	auto result = IDENTATION() + std::string("Except(") + ToString(ex->type) + "," + ex->name +
				  "):" + NEWLINE();
	INDENT();
	List(ex->body, result);
	DEDENT();
